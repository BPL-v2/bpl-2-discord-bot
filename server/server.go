...
func SetRoutes(r *gin.Engine, session *discordgo.Session, bplClient *client.ClientWithResponses) {
	...
	group.Handle("GET", ":guild_id/members/:discord_id", getMemberByDiscordID(session))
}

func getMemberByDiscordID(session *discordgo.Session) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if discordID == "" {
			c.JSON(400, gin.H{"message": "missing discord_id"})
			return
		}
		member, err := session.GuildMember(guildID, discordID)
		if err != nil {
			c.JSON(500, gin.H{"message": fmt.Sprintf("error getting member: %v", err)})
			return
		}
		c.JSON(200, member)
	}
